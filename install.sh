...
SMB="" SMB_USER="" NFS="" AUTO_UPDATE=0
...
sudo apt-get install -y -qq python3 python3-rpi.gpio omxplayer raspi-gpio git cifs-utils nfs-common

say "Config"
[ -f "$HERE/config.json" ] || cp "$HERE/config.example.json" "$HERE/config.json"
mkdir -p "$HERE/state" "$HERE/videos"
chmod +x "$HERE/player.py" "$HERE/buttons.py" "$HERE/tvctl" "$HERE/update.sh"
...
say "Services"
for unit in tvplayer.service tvbutton.service tvupdate.service tvupdate.timer; do
    sed "s#__DIR__#$HERE#g" "$HERE/systemd/$unit" | sudo tee "/etc/systemd/system/$unit" >/dev/null
done
sudo systemctl daemon-reload
sudo systemctl enable tvplayer.service tvbutton.service
sudo systemctl restart tvbutton.service tvplayer.service
